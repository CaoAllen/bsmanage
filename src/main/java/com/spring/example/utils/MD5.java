...
public class MD5 {
	...
	/*
	 * Temporary variables of ABCD
	 */
	private int Atemp, Btemp, Ctemp, Dtemp;

	/*
	 * Constant Ti formula:floor(abs(sin(i+1))×(2pow32)
	 */
	...
	/*
	 * A left shift, calculating method of unknown
	 */
	...
	private int shift(int a, int s) {
		return (a << s) | (a >>> (32 - s));// When you move to the right, the high point must be filled, not added to the symbol bit
	}
	...
	/*
	 * Fill function Should be met after treatment bits=448(mod512),Byte is bytes=56（mode64) Fill mode is Add one 0 first, Other bit zero padding
	 * Finally, add the original length of 64 bits
	 */
	private int[] add(String str) {
		int num = ((str.length() + 8) / 64) + 1;// A group of 512 bits and 64 bytes
		int strByte[] = new int[num * 16];// 64/4=16,So there are 16 integers
		for (int i = 0; i < num * 16; i++) {// Initialize all 0
			strByte[i] = 0;
		}
		int i;
		for (i = 0; i < str.length(); i++) {
			strByte[i >> 2] |= str.charAt(i) << ((i % 4) * 8);// An integer store four bytes, small endian
		}
		strByte[i >> 2] |= 0x80 << ((i % 4) * 8);// Tail add 1
		/*
		 * Add the length of the original length to the length of the digit,
		 * So take 8,Then the small endian,So put it in the bottom second,
		 * Only 32 bits are used here
		 */
	...
	/*
	 * Integer into 16 hexadecimal string
	 */
